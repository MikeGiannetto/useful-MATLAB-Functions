...
% the frequency at which images or lines were acquired. If noisy~=0, data
% are visualized using imagei.m. If "infile" contains a two-dimensional
% movie and the directory containing "infile" also contains a file called
% 'reg.mat', images are registered according to its contents (see
% reg_raw.m); info.isReg indicates whether images have been registered.
% Requires read_Thor_xml.m. Requires imagei.m for visualization. 
...
% Next: Make images look the same as they look on the microscope screen!
% Need ndgrid instead of meshgrid? Eliminate permute(...,[2 1 3])? Are
% info.ImageHeight and info.ImageWidth switched when handling line scans?
...
if ~exist('frlist','var') || isempty(frlist)
    frlist=1:info.AllocatedFrames; % by default, read all frames
else
...
if info.isLineScan
    ...
    coords.Y = [];
    info.FrameRate = info.FrameRate*info.ImageHeight; % lines/s
    coords.T = (lnlist-1)/info.FrameRate;
